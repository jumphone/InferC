...
while(i<=nrow(LIST_COM)){
    ...
    while(j<=length(this_all_path_list)){
        ...
        ###############################
        this_pbmc_data=this_pbmc@assays$RNA@counts
        this_tmp=stringr::str_split(this_path,'/')[[1]]
        this_pbmc=CreateSeuratObject(counts = this_pbmc_data, project = this_tmp[length(this_tmp)], min.cells = 0, min.features = 0)
        this_pbmc <- NormalizeData(
            object = this_pbmc,
            assay = 'RNA',
            normalization.method = 'LogNormalize',
            scale.factor = median(this_pbmc$nCount_RNA)
            )
        ##############################
    ...
    saveRDS(pbmc, paste0('/home/database/data/COM_scATAC/data/',this_organ,'.seurat.GeneActivity.rds'))
    print(i)
    i=i+1
    }
