...
pdf('plot/p07_scATAC_umap.pdf',width=4,height=4)
DimPlot(pbmc,group.by='type',label=TRUE)+NoLegend()
DimPlot(pbmc,group.by='type',label=FALSE)+NoLegend()
dev.off()
...
MAT=CELLBIN$mat[,order(as.numeric(colnames(CELLBIN$mat)))]
LOOP=rownames(CONNS)
NET=inferloop.splitLoop(LOOP)


source('InferC.R')
inferc.out=inferc.calLoopScore(MAT, NET[1:10000,])
############################
...
#############
pdf('plot/p07_PDGFRA_track_lls.pdf',width=18,height=5)
...
#LOOP_TYPE = pnorm(inferc.generate_mean(LOOP, TYPE)) * s2_loop$gls
#LOOP_TYPE=Seurat::FastRowScale(LOOP_TYPE)
#LOOP_TYPE[which(is.na(LOOP_TYPE))]=0
